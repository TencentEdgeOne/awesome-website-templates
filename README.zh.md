<div align="center">
  <strong><a href="https://github.com/TencentEdgeOne/awesome-website-templates/blob/main/README.zh.md"> 中文简体 </a> ｜ English</strong>
</div>
<hr/>
<h1 align="center">📢 很棒的网站模板集合 📢</h1>
<p align="center"><i>✨ 各类精选项目，支持EdgeOne Pages一键部署，持续更新... ✨</i></p>
<p align="center"><i>✨ 欢迎开发者在讨论区推荐项目！ ✨</i></p>

  <div align="center">
    <table>
      <tr bgcolor="#f0f0f0"><th><a href="#game">Game</a></th>
<th><a href="#scaffolds">Scaffolds</a></th>
<th><a href="#designdrawingprojects">Design & Drawing Projects</a></th>
<th><a href="#threed">ThreeD</a></th>
<th><a href="#cooldiy">Cool DIY</a></th>
    </tr>
</table>
</div><h1 id='game'>Game</h1>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Hextris" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/7ec1a2ed-6021-404d-9048-bf3171412ed7.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Hextris · <a href="https://github.com/Hextris/hextris">Github👆</a></h3><div> <br/>
    </div>
      <div><em>这是一款受俄罗斯方块启发的快节奏益智游戏。Hextris的玩法基本与俄罗斯方块相同，通过控制方向来消除下落的方块。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/Hextris/hextris&output-directory=.%2F" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Pacman" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/aa3fc4ab-4ac1-422d-aab9-fc12893528af.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Pacman · <a href="https://github.com/mumuy/pacman">Github👆</a></h3><div> <br/>
    </div>
      <div><em>一个纯前端制作的吃豆人游戏。适合初学者学习和娱乐。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/mumuy/pacman&output-directory=.%2F" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="A Dark Room" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/86442241-cd3d-49d2-83f5-bf119ea2b8a8.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>A Dark Room · <a href="https://github.com/doublespeakgames/adarkroom">Github👆</a></h3><div> <br/>
    </div>
      <div><em>醒来。头痛欲裂。视线模糊。来点燃火焰吧。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/doublespeakgames/adarkroom&build-command=npm%20install&output-directory=.%2F" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table><h1 id='scaffolds'>Scaffolds</h1>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Tencent RTC" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/2b73ba30-ee33-4e02-b874-644e53b65751.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Tencent RTC · <a href="https://github.com/Tencent-RTC/TUIRoomKit">Github👆</a></h3><div> <br/>
    </div>
      <div><em>腾讯音视频通讯Web客户端。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/Tencent-RTC/TUIRoomKit&root-directory=.%2FWeb%2Fexample%2Fwebpack-vue2.7-ts" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Astrofy" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/b8fa34fc-9bd3-48aa-b226-4e611405fa47.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Astrofy · <a href="https://github.com/manuelernestog/astrofy">Github👆</a></h3><div> <br/>
    </div>
      <div><em>Astrofy是一个使用Astro和TailwindCSS构建的免费开源个人作品集网站模板。可以在几分钟内创建一个带有博客的网站。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/manuelernestog/astrofy" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Versatile Landing, Blog & Docs Template for a Simplified Web Presence" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/7afde78c-f9af-4a8f-b105-55ee4201f066.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Versatile Landing, Blog & Docs Template for a Simplified Web Presence · <a href="https://github.com/mearashadowfax/ScrewFast">Github👆</a></h3><div> <br/>
    </div>
      <div><em>ScrewFast是一个开源模板，设计用于快速高效地搭建网络项目，将极简主义与功能性融为一体。无论您是展示作品集、推出公司落地页，还是运营博客，ScrewFast都能提供您所需的一切。通过结合Astro、Tailwind CSS和Preline UI的强大功能，这个模板为您的网络呈现提供了实用且美观的解决方案。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/mearashadowfax/ScrewFast" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="shadcn-admin" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/fe213e6c-3758-464c-9b5a-5b1a527bdaa5.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>shadcn-admin · <a href="https://github.com/satnaing/shadcn-admin">Github👆</a></h3><div> <br/>
    </div>
      <div><em>使用Shadcn和Vite精心打造的管理后台UI。在设计时考虑了响应性和可访问性。我一直在工作和个人项目中创建仪表盘UI，总想为未来的项目制作一个可重用的仪表盘UI集合；现在它终于问世了。虽然我创建了一些自定义组件，但部分代码直接改编自ShadcnUI示例。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/satnaing/shadcn-admin&output-directory=dist&install-command=pnpm%20install&build-command=pnpm%20build" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="vue-manage-system" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/7b1c3f9e-7946-43f7-acd9-1a4951404233.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>vue-manage-system · <a href="https://github.com/lin-xin/vue-manage-system">Github👆</a></h3><div> <br/>
    </div>
      <div><em>基于Vue的管理后台脚手架。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/lin-xin/vue-manage-system" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table><h1 id='designdrawingprojects'>Design & Drawing Projects</h1>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Mini-Cover" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/10489988-8f16-4cc1-a6cd-3848dca5b324.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Mini-Cover · <a href="https://github.com/JLinMr/Mini-Cover">Github👆</a></h3><div> <br/>
    </div>
      <div><em>Mini-Cover是一个现代化的封面生成工具，专为博客、短视频和社交媒体设计。它支持多种自定义选项，让您轻松创建个性化封面图片。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/JLinMr/Mini-Cover&project-name=mini-cover&repository-name=mini-cover" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Coverview" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/b8701802-5fad-4270-81a1-859e90816114.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Coverview · <a href="https://github.com/rutikwankhade/CoverView">Github👆</a></h3><div> <br/>
    </div>
      <div><em>为您的博客创建封面图片现在超级简单。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/rutikwankhade/CoverView" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table><h1 id='threed'>ThreeD</h1>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt=" My Room in 3D" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/2079a8d6-6a4e-4b66-9a60-156a87b02c26.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3> My Room in 3D · <a href="https://github.com/brunosimon/my-room-in-3d">Github👆</a></h3><div> <br/>
    </div>
      <div><em>使用网页3D技术构建的精美房间。这个项目非常有趣，可以作为展示网页3D技术的完美演示。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/brunosimon/my-room-in-3d&project-name=3droom&repository-name=3droom&install-command=npm%20update%20%26%26%20npm%20run%20build" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="3D Blog" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/9709bdfd-93e5-41cc-8a8a-ed87e519bd6a.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>3D Blog · <a href="https://github.com/brunosimon/folio-2019">Github👆</a></h3><div> <br/>
    </div>
      <div><em>一个使用Three.js实现的3D博客项目，用户可以驾驶一辆小汽车来寻找和阅读文章。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/brunosimon/folio-2019&output-directory=dist&install-command=npm%20install&build-command=npm%20run%20build" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="3D Apple iphone in React JS" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/f4c732f7-9a1d-4d75-bd49-f7bb3b668a37.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>3D Apple iphone in React JS · <a href="https://github.com/codebucks27/3D-Landing-page-for-Apple-iPhone">Github👆</a></h3><div> <br/>
    </div>
      <div><em>在React JS中为苹果iPhone创建一个令人惊叹的3D登陆页面。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/codebucks27/3D-Landing-page-for-Apple-iPhone" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table><h1 id='cooldiy'>Cool DIY</h1>  
<table>
    <td width=350>      
    <img width="300" object-fit="fill" alt="Pokémon Cards" src="https://cloudcache.tencent-cloud.com/qcloud/ui/static/static_source_business/34e2ff56-8730-4a52-9717-340ccef501b0.png" style="margin-right: 24px"/></td>
    <td width=700><div>
      <h3>Pokémon Cards · <a href="https://github.com/simeydotme/pokemon-cards-css">Github👆</a></h3><div> <br/>
    </div>
      <div><em>使用CSS变换、渐变、混合模式和滤镜来模拟宝可梦剑盾时代交换卡片中的各种全息效果。</em><div> <br/>
      <div>
        <a href="https://edgeone.ai/pages/new?from=awesome&template=https://github.com/simeydotme/pokemon-cards-css&output-directory=dist&install-command=npm%20install&build-command=npm%20run%20build" target="_blank" style="text-decoration: none;">🚀 一键部署</a>
      <div> <br/></td>
  </tr>
</table></div>